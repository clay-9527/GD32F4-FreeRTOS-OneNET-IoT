...
_Bool OneNET_RegisterDevice(void)
{
	...
	if(data_ptr)
	{
		...
		{
			debug_printf("create device: %s, %s, %d, %s\r\n", devid, name, pid, key);
			result = 0;
		}
	}
	...
	if(MQTT_PacketConnect(PROID, authorization_buf, DEVICE_NAME, 256, 1, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		if(dataPtr != NULL)
		{
			if(MQTT_UnPacketRecv(dataPtr) == MQTT_PKT_CONNACK)
			{
				...
				{
					case 0:debug_printf("Tips: 连接成功\r\n");status = 0;break;
					
					case 1:debug_printf("WARN: 连接失败：协议错误\r\n");break;
					case 2:debug_printf("WARN: 连接失败：非法的clientid\r\n");break;
					case 3:debug_printf("WARN: 连接失败：服务器失败\r\n");break;
					case 4:debug_printf("WARN: 连接失败：用户名或密码错误\r\n");break;
					case 5:debug_printf("WARN: 连接失败：非法链接(比如token非法)\r\n");break;
					
					default:debug_printf("ERR: 连接失败：未知错误\r\n");break;
				}
			}
		}
...
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"humi\":{\"value\":%d},", temp);
//	strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"temp\":{\"value\":%d},", humi);
//	strcat(buf, text);
	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"MQ2\":{\"value\":%d},", MQ2);
//	strcat(buf, text);
...
//	UsartPrintf(USART_DEBUG, "Tips:	OneNet_SendData-MQTT\r\n");
	...
	if(body_len)
	{
		if(MQTT_PacketSaveData(PROID, DEVICE_NAME, body_len, NULL, &mqttPacket) == 0)				//封包
		{
			...
			debug_printf("WARN: EDP_NewBuffer Failed\r\n");
	}
...
void OneNET_Publish(const char *topic, const char *msg)
{
	...
	debug_printf("Publish Topic: %s, Msg: %s\r\n", topic, msg);
...
void OneNET_Subscribe(void)
{
	...
	debug_printf("Subscribe Topic: %s\r\n", topic_buf);
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				debug_printf("topic: %s, topic_len: %d, payload: %s, payload_len: %d\r\n",
   ...
   debug_printf("CLOUD CTRL LED=%d\r\n", ledNewState);
		...
		case MQTT_PKT_PUBACK:														//发送Publish消息，平台回复的Ack
			...
			if(MQTT_UnPacketPublishAck(cmd) == 0)
				debug_printf("Tips: MQTT Publish Send OK\r\n");
		...
		case MQTT_PKT_SUBACK:																//发送Subscribe消息的Ack
			...
			if(MQTT_UnPacketSubscribe(cmd) == 0)
				debug_printf("Tips: MQTT Subscribe OK\r\n");
			else
				debug_printf("Tips: MQTT Subscribe Err\r\n");
